...
async function loadScenarioData() {
  ...
  const response = await fetch(
    `${GITHUB_PAGES_BASE_URL}/scenarioData/메인.json`
  );
  ...
  const passDataResponse = await fetch(
    `${GITHUB_PAGES_BASE_URL}/PassData/passData.json`
  );
...
async function loadAllScenarios() {
  ...
  for (const file of scenarioFiles) {
    const response = await fetch(
      `${GITHUB_PAGES_BASE_URL}/scenarioData/${encodeURIComponent(file)}`
    );
...
class ScenarioChecker {
  ...
  async loadScenarioFile(fileName) {
    if (!this.fileToIdSetMap[fileName]) {
      const response = await fetch(
        `${this.baseUrl}/scenarioData/${encodeURIComponent(fileName)}`
      );
      const data = await response.json();
...
// 사용 예시
const checker = new ScenarioChecker(
  "https://[username].github.io/peerline-sceanario/data"
);
  ...
  useEffect(() => {
    async function fetchData() {
      const GITHUB_PAGES_BASE_URL =
        "https://[username].github.io/peerline-sceanario/data";
      const response = await fetch(
        `${GITHUB_PAGES_BASE_URL}/scenarioData/${scenarioName}.json`
      );
